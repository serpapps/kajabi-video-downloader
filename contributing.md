...
### 5.4 Video Information Extraction

#### 5.4.1 Metadata Commands
```bash
...
#### 5.4.2 Quality Control Commands
```bash
# Download best quality MP4
...
#### 8.4.1 YAML Configuration Template
```yaml
# config.yaml
loom_downloader:
  output:
...
```bash
# Test connection speed to CDN
test_cdn_speed() {
    local video_id="$1"
    local test_file="https://cdn.loom.com/sessions/$video_id/transcoded/mp4/720/video.mp4"
    
    echo "Testing download speed to Loom CDN..."
    
    # Download first 10MB to test speed
    curl -r 0-10485760 -w "Average speed: %{speed_download} bytes/sec\nTime: %{time_total}s\n" -o /dev/null "$test_file"
}
...
#### 9.2.2 Memory Usage Optimization
```bash
...
efficient_download() {
    ...
    # Use curl with progress bar and limited buffer
    curl --progress-bar --limit-rate 2M -o "$output_file" "$url"
}
...
check_disk_space() {
    ...
    if [ "$available_space" -lt "$min_space_kb" ]; then
        echo "✗ Insufficient disk space. Available: $(($available_space / 1024 / 1024))GB, Required: ${min_space_gb}GB"
        return 1
    fi
...
### 9.3 Video Quality and Corruption Issues
...
verify_video_integrity() {
    ...
    if ffprobe -v error -select_streams v:0 -show_entries stream=codec_name -of csv=p=0 "$file_path" >/dev/null 2>&1; then
        echo "✓ Video file integrity verified: $file_path"
        return 0
    else
        echo "✗ Video file appears to be corrupted: $file_path"
        return 1
    fi
}
...
get_video_info() {
    ...
    echo "Video Information for: $file_path"
    echo "=================================="
    ...
    # Resolution
    local resolution=$(ffprobe -v quiet -select_streams v:0 -show_entries stream=width,height -of csv="s=x:p=0" "$file_path")
    echo "Resolution: $resolution"
    
    # Codec information
    local video_codec=$(ffprobe -v quiet -select_streams v:0 -show_entries stream=codec_name -of csv="p=0" "$file_path")
    local audio_codec=$(ffprobe -v quiet -select_streams a:0 -show_entries stream=codec_name -of csv="p=0" "$file_path")
    echo "Video codec: $video_codec"
    echo "Audio codec: $audio_codec"
}
```

#### 9.3.2 Automatic Repair Attempts
```bash
...
repair_mp4() {
    ...
    if ffmpeg -err_detect ignore_err -i "$input_file" -c copy "$output_file"; then
        ...
        if verify_video_integrity "$output_file"; then
...
fix_timestamps() {
    ...
    # Fix negative timestamps and other timing issues
...
optimize_for_streaming() {
    ...
    # Move moov atom to front for fast start
...
- **parallel**: For batch processing and concurrent downloads
